...
label_text = [f"{label}: {value} EUR - {percent:.2f}%" for label, value, percent in zip(assets.keys(), assets.values(), [x / kokku_varad * 100 for x in assets.values()])]
...
fig.update_layout(
    title='Finance Portfolio Overview',
    showlegend=True
)
# title to center
